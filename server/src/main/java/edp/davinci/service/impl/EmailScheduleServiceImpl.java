...
public class EmailScheduleServiceImpl implements ScheduleService {
    ...
    private List<ImageContent> generateImages(long jobId, CronJobConfig cronJobConfig, Long userId) throws Exception {
        ...
        for (CronJobContent cronJobContent : cronJobConfig.getContentList()) {
            if("portal".equalsIgnoreCase(cronJobContent.getContentType())){
                List<Dashboard> dashboards = dashboardMapper.getByPortalId(cronJobContent.getId());
                for(Dashboard dashboard : dashboards){
                    CronJobContent newContent = new CronJobContent();
                    newContent.setContentType(cronJobContent.getContentType());
                    newContent.setId(dashboard.getId());
        ...
        if (!CollectionUtils.isEmpty(imageContents)) {
            screenshotUtil.screenshot(userId, jobId, imageContents);
        }
    ...
    public List<ImageContent> getPreviewImage(Long userId, String contentType, Long contentId) throws Exception{
        ...
        if (!CollectionUtils.isEmpty(imageContents)) {
            screenshotUtil.screenshot(userId, 0, imageContents);
        }
        return imageContents;
    }
    ...
    public String getContentUrl(Long userId, String contentType, Long contengId) {
        ...
        sb.append(serverUtils.getServerUrl())
