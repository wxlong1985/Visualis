...
public class DisplayServiceImpl extends VizCommonService implements DisplayService {
    ...
    @Override
    public Display copyDisplay(DisplayInfo displayInfo, User user) {
        ...
        display.setId(null);
        display.setName(displayInfo.getName());
        display.setDescription(displayInfo.getDescription());
        display.setPublish(displayInfo.getPublish());

        int insert = displayMapper.insert(display);
        if (insert > 0) {
            List<DisplaySlide> displaySlides = displaySlideMapper.selectByDisplayId(displayInfo.getTargetId());
            for(DisplaySlide displaySlide : displaySlides){
                List<MemDisplaySlideWidget> oldMemSlideWidgets = memDisplaySlideWidgetMapper.getMemDisplaySlideWidgetListBySlideId(displaySlide.getId());
                displaySlide.setId(null);
                displaySlide.setDisplayId(display.getId());
                displaySlideMapper.insert(displaySlide);
                for(MemDisplaySlideWidget memDisplaySlideWidget : oldMemSlideWidgets){
                    memDisplaySlideWidget.setId(null);
                    memDisplaySlideWidget.setDisplaySlideId(displaySlide.getId());
                }
                if(oldMemSlideWidgets.size() > 0){
                    memDisplaySlideWidgetMapper.insertBatch(oldMemSlideWidgets);
                }
            }
            return display;
        } else {
